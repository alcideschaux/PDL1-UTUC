---
title: PDL1 positivity, median 1%
...
data$outcome <- data$PDL1_pos_1_median
```
...
ggplot(data, aes(x = pt, fill = outcome)) +
  ...
  annotate("text", x = 3, y = 40, label = paste("P value =", p)) +
...
ggplot(data, aes(x = grade, fill = outcome)) +
  geom_bar(position = position_dodge(), color = "black") +
  scale_y_continuous(limits = c(0, 40), breaks = seq(0, 40, 10)) +
  annotate("text", x = 2, y = 40, label = paste("P value =", p)) +
