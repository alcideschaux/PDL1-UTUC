---
title: PDL1 positivity, maximum 5%
...
data$outcome <- data$PDL1_pos_5_max
```
...
ggplot(data, aes(x = location, fill = outcome)) +
  ...
  annotate("text", x = 2, y = 45, label = paste("P value =", p)) +
...
ggplot(data, aes(x = pt, fill = outcome)) +
  geom_bar(position = position_dodge(), color = "black") +
  scale_y_continuous(limits = c(0, 45), breaks = seq(0, 45, 10)) +
  annotate("text", x = 3, y = 45, label = paste("P value =", p)) +
...
ggplot(data, aes(x = high_grade, fill = outcome)) +
  ...
  annotate("text", x = 1.5, y = 80, label = paste("P value =", p)) +
...
with(data, gsurv(fu_mo, dod, outcome)) +
  ...
  annotate("text", label = paste("P value =", p), x = 25, y = .3) +
